...
describe('SignalNexus', () => {
  it('switches horizon and selects a market token', () => {
    ...
    expect(screen.getByRole('button', { name: /Select ALPHA.*\+48\.0%/i })).toBeInTheDocument()
  ...
  it('filters a narrative and inspects a real mint-to-creator pair', () => {
    ...
    const narrative = screen.getByRole('button', { name: /Filter narrative AI \/ COMPUTE/i })
    fireEvent.click(narrative)
    expect(narrative).toHaveClass('is-selected')
    fireEvent.click(screen.getByRole('button', { name: /Inspect new token FRESH/i }))
    expect(screen.getByText('NEW MINT OBSERVED')).toBeInTheDocument()
    expect(screen.getByRole('heading', { name: '$FRESH' })).toBeInTheDocument()
    expect(screen.getByText(/mint ↔ creator edge comes directly from Pump.fun program logs/i)).toBeInTheDocument()
  })
})
