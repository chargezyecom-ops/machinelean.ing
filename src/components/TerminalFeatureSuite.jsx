    ...
    <div className="feature-suite__frame">
      ...
      <div className="feature-console">
        ...
        <div className="feature-output-grid">
          ...
          <aside className="feature-evidence"><div className="feature-evidence__head"><span>MACHINE EVIDENCE LEDGER</span><CircleDot size={12} /></div><dl><div><dt>Entity</dt><dd>${token.symbol}</dd></div><div><dt>24h flow</dt><dd>${Math.round(token.volume24).toLocaleString()}</dd></div><div><dt>Depth</dt><dd>${Math.round(token.liquidity).toLocaleString()}</dd></div><div><dt>Source</dt><dd>{token.source}</dd></div><div><dt>Channel</dt><dd>{active.endpoint || 'browser/local'}</dd></div><div><dt>State</dt><dd>{active.mode}</dd></div></dl>{active.mode !== 'local' && !researchApiEnabled && <div className="feature-abstain"><ShieldAlert size={14} /><span><b>OBSERVATION CHANNEL MISSING</b>The Machine abstains until wallet, social or trained-model evidence is available.</span></div>}</aside>
        </div>
