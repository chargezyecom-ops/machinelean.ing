...
export default function PumpHistoricalLab() {
  ...
  return <section className="history-lab" id="history-lab" aria-labelledby="history-lab-title">
    <div className="history-lab__head">
      <div><span><Database size={14} /> D0 / MACHINE MEMORY ARCHIVE</span><h2 id="history-lab-title">The Machine remembers <em>every launch.</em></h2><p>Reconstruction multi-horizon des événements Pump.fun, de leurs identités wallet et de leur propagation mémétique avant résolution par le marché.</p></div>
      <div className="history-lab__badge"><ShieldAlert size={15} /><span><b>RECONSTRUCTED MEMORY</b>SIMULATED / NO LIVE CLAIM</span></div>
    </div>

    <div className="history-commandbar">
      <div><span>MEMORY WINDOW</span>{Object.keys(historicalWindows).map((key) => <button className={windowKey === key ? 'is-active' : ''} type="button" onClick={() => setWindowKey(key)} key={key}>{key}</button>)}</div>
      <div><span>RESOLUTION HORIZON</span>{Object.keys(horizonProfiles).map((key) => <button className={horizon === key ? 'is-active' : ''} type="button" onClick={() => setHorizon(key)} key={key}>{key}</button>)}</div>
      <strong><i /> MEMORY INDEXED / {researchWindow.coverage}</strong>
    </div>

    <div className="history-metrics">
      <HistoricalMetric icon={Activity} label="ENTITIES REMEMBERED" value={compact.format(researchWindow.launches)} detail={`${compact.format(researchWindow.observations)} temporal states`} tone="green" />
      <HistoricalMetric icon={Users} label="IDENTITIES RESOLVED" value={compact.format(researchWindow.creators)} detail={`${compact.format(researchWindow.transactions)} transactions`} />
      <HistoricalMetric icon={Target} label={`${horizon} RELEVANCE PRECISION`} value={`${profile.precision}%`} detail={`median forewarning ${profile.lead}`} tone="blue" />
    ...
    <div className="history-core-grid">
      <div className="history-panel history-funnel">
        <div className="history-panel__head"><span>D1 / RELEVANCE CLASSIFICATION FUNNEL</span><b>{horizon} EVENT PATH</b></div>
      ...
      <div className="history-panel history-labels">
        <div className="history-panel__head"><span>D2 / EVENT CLASSIFICATION MATRIX</span><b>SUPERVISION TARGETS</b></div>
    ...
    <div className="history-narrative-grid">
      <div className="history-panel history-narratives">
        <div className="history-panel__head"><span>D3 / MEMETIC EVENT DETECTOR</span><b>SEMANTIC VELOCITY / 24H</b></div>
      ...
      <aside className="history-panel history-narrative-focus">
        <div className="history-panel__head"><span>D4 / RELEVANT CLUSTER READOUT</span><BrainCircuit size={13} /></div>
    ...
    <div className="history-bottom-grid">
      <div className="history-panel history-influence">
        <div className="history-panel__head"><span>D5 / PERSONS OF INTEREST</span><b>WALLET × KOL IDENTITY GRAPH</b></div>
      ...
      <div className="history-panel history-cohorts">
        <div className="history-panel__head"><span>D6 / EVENT MEMORY LEDGER</span><b>LABEL COMPLETENESS / 100%</b></div>
